...
class Student extends Server
{
    ...
    public function set_status($id = "", $status = "")
    {
        ...
        if ($status == "1" || $status == "0") {
            ...
            if ($newStatus == "error") {
                ...
                $updateStatus = $this->crud(
                    "update alumnos set estado = '$newStatus' where id = '$id'",
    ...
    public function get_one($idStudent = 0)
    {
        ...
        $search = $this->search(
            $this->querySelect . " where alumnos.id='$idStudent'",
        ...
        if ($search["status"] == "done") {
            ...
            return $this->swicth_status_query(
                $search["status"],
                "",
    ...
    public function get_all()
    {
        ...
        $search = $this->search(
            $this->querySelect . " order by alumnos.nombre desc",
    ...
    public function get_by_name($name = "")
    {
        ...
        if (!$this->forms->empty_data($name)) {
            ...
            $search = $this->search(
                $this->querySelect . " where (alumnos.nombre like '%$name%' or alumnos.apellido like '%$name%') or (alumnos.nombre regexp '$busqueda' && alumnos.apellido regexp '$busqueda') order by alumno desc",
    ...
    public function admin_student_table($data = "")
    {
        ...
        while ($datos = mysqli_fetch_assoc($data)) {
            ...
            $bodyTable = $bodyTable . "<tr>
                        ...
                        <td class='text-center'>
                            <button class='reset-passBtn' id='reset-passBtn$datos[id]' value='$datos[id]' title='Click para resetear contraseña'>
    ...
    public function add_one($data = "")
    {
        ...
        if (isset($data) && $this->forms->empty_data($data)) {
            ...
            if (
                !$this->forms->empty_data($usuario) ||
                !$this->forms->empty_data($cargos) ||
                ...
                $cargos = $this->data_cleaner($cargos);
                ...
                $search = $this->search(
                    "select id from alumnos where usuario='$usuario'",
...
$html .= '</select>
        ...
        <div class="footer-form">
            ...
            <a href="index.php" id="add-student" class="btn action-btn" tabindex="7" role="button" aria-pressed="true">Iniciar sesion</a>
        </div>
    </form>
</div>';
    ...
    public function set_student($data = "")
    {
        ...
        if (isset($data) && $this->forms->empty_data($data)) {
            ...
            if (
                ...
                $setOne = $this->crud(
                    "update alumnos set nombre='$name', apellido='$apellido', usuario='$user' where id='$idStudent'",
    ...
    private function get_data_form_set($idStudent = "")
    {
        ...
        $getData = $this->search(
            "Select usuario, nombre, apellido from alumnos where id='$idStudent'",
    ...
    public function del_student($idStudent = "")
    {
        ...
        if ($this->forms->empty_data($idStudent)) {
            ...
            $delStudent = $this->crud(
                "delete from alumnos where id='$idStudent'",
            ...
            $delEr = $this->crud(
                "delete from examenes_realizados where er_alumno='$idStudent'",
    ...
    public function reset_password($idStudent = "")
    {
        ...
        if ($this->forms->empty_data($idStudent)) {
            ...
            $resetPass = $this->crud(
                "update  alumnos set password='$password' where id='$idStudent'",
    ...
    public function set_profile_data(
        ...
        if (
            $this->forms->empty_data($studentId) &&
            $this->forms->empty_data($user) &&
            $this->forms->empty_data($name) &&
            $this->forms->empty_data($apellidos)
        ) {
            ...
            $update = $this->crud(
                "update alumnos set nombre = '$name', usuario = '$user', apellido='$apellidos', fechaDeActualizacion='$date' where id='$studentId'",
    ...
    public function set_student_password(
        ...
        if (
            ...
            if ($passVerify["data"] != true) {
                ...
                $update = $this->crud(
                    "update alumnos set password = '$password', fechaDeActualizacion='$date' where id='$studentId'",
