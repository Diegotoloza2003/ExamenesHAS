<?php 
	...
	class Login extends Server{
			...
			if($this -> forms -> empty_data($password)){
				...
				if (strlen($password) < 4 ) {
					...
					$status = $this -> status_query("no-data", 
						                            "Su Contraseña debe contener un minimo de 8 caracteres", 
						                            false);
						
				}else if (strlen($password) > 12 ) {
					
					$status = $this -> status_query("no-data", 
						                            "Su Contraseña debe contener un maximo de 12 caracteres",
						                            false);

				}else if (!preg_match('`[a-z]`', $password)) {
					
					$status = $this -> status_query("no-data", 
						                            "Su Contraseña debe contener minimo menos una letra minuscula", 
						                            false);
					
				}else if (!preg_match('`[A-Z]`', $password)) {
					
					$status = $this -> status_query("no-data", 
						                            "Su Contraseña debe contener minimo una letra mayuscula", 
						                            false);
					
				}else if (!preg_match('`[0-9]`', $password)) {
					
					$status = $this -> status_query("no-data", 
						                            "Su Contraseña debe contener minimo un caracter numerico",
						                            false);
		...
		public function is_user_logged(){
			...
			//verificacion de usuario Registrados
		...
		public function is_user_type($userType = ""){
			...
			ejemplo: si es la pagina de inicio del admin se coloca is_user_type('admin') y dependiendo del tipo de usuario Registrados
